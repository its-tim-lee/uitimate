...
<Details>
  <DetailsTrigger>
    **DEEP DIVE**: If you notice that this is very different from **<u>Shadcn</u>**’s Sidebar...*
  </DetailsTrigger>
  <DetailsContent>
...
Our `<Sidebar>` will use [@uitimate/drawer](./../../drawer/introduction) (we call it "mobile sidebar")
...
<VersatileTabs2 defaultValue="sidebar-rwd-minimum-demo" compact>
  <VersatileTabs2List>
      <VersatileTabs2Trigger value="sidebar-rwd-minimum-demo">Preview</VersatileTabs2Trigger>
  </VersatileTabs2List>
...
<Banner>
  ...
  For those who wonders why of that and why create the `<Sidebar.Mobile>` & `<Sidebar.Desktop>`, check the source code.
</Banner>
...
It’s actually the easiest. You just need to provide the prop `enableMobileSidebar` to `false` on `<SidebarLayout>`
(so no need to use either `<Sidebar.Mobile>` or `<Sidebar.Desktop>` anymore).

<VersatileTabs2 defaultValue="sidebar-toggle-control" id="no-mobile-sidebar-demo" compact>
  <VersatileTabs2List>
    <VersatileTabs2Trigger value="sidebar-toggle-control">Preview</VersatileTabs2Trigger>
  </VersatileTabs2List>
  <VersatileTabs2Content
...
Now, we need to explain where you should place `<Sidebar>` in `<SidebarLayout>`. It only matters in desktop breakpoint:
```tsx file=./Sidebar.meta.tsx#L4-L14
```
...
Let's! For more on its family components to use, and more demos on showing how versatile this component can be, check [the API reference page](./../api)
