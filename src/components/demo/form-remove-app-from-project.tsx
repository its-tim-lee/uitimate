...
import { Form, FormLabel, FormControl, FormItem } from "../ui/Form/Form"
  ...
  return (
    <>
      <Dialog open={isOpen} onClose={() => setIsOpen(false)} className='tw:text-sm tw:p-0'>
        ...
        <main className='tw:p-4'>
          <p>Details about the app that will be removed:</p>
          <br />
          <p className='tw:font-semibold'>I understand that removing this app will result in permanent deletion after 30 days. I also understand that the following will happen:</p>
          <br />
        ...
        <Separator className='tw:my-0' />

        <DialogAction className='tw:p-4'>
          <Cta variant='ghost' type='submit' onClick={() => setIsOpen(false)}>Cancel</Cta>
          <Cta variant='secondary' disabled={shouldStopToNextStep} type='submit' onClick={removeApp}>Remove App</Cta>
        </DialogAction>
      </Dialog>


      <Cta onClick={() => setIsOpen(true)} variant="outline">
        Remove this app
      </Cta>
    </>
  )
